...
void error_box(const std::string &message) {
	MessageBoxA(nullptr, message.c_str(), "D3D11 Proxy Error", MB_OK);
}

static bool load_winmm() {
	char systemPath[MAX_PATH];
	if (!GetSystemDirectoryA(systemPath, MAX_PATH)) {
		error_box("Could not access System32 path.");
	}
	...
	if (!winmm_dll) {
		...
		if (!GetSystemWow64DirectoryA(systemPathWow64, MAX_PATH)) {
			error_box("Could not access SysWOW64 path.");
		}
		strcat_s(systemPathWow64, "\\winmm.dll");

		winmm_dll = LoadLibraryA(systemPathWow64);
		if (!winmm_dll) {
			error_box("Could not load org 'winmm.dll' from '" + std::string(systemPathWow64) + "' or '" + std::string(systemPath) + "'");
			return false;
		}
	}
...
typedef int (*RevitaLoaderEntryFunc)();

// ReSharper disable once CppDFAConstantFunctionResult
DWORD WINAPI OnProcessAttach([[maybe_unused]] LPVOID lpParam) {
	if (!load_winmm())
		return 0;
